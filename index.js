...
const margin = 90;
const ratio = width / (height - margin);
const rotate = -9.9;
...
const countryDisplay = d3.geoPath().projection(contrast);
// this is to display all the countries

const colorScale = d3.scaleSqrt(["#CCD6FB", "#B705D4", "#71001E"]);
...
let generate = (promiseFunc) => {
    Promise.all(promiseFunc).then(function ([countries, data]) {
        ...
        let sortedHighest = data.sort(
            (a, b) =>
                b["Cases in the last 7 days"] - a["Cases in the last 7 days"]
                // sorted functions to show the highest cases
        );
        let top = sortedHighest.slice(0, 5);
        let bottom = sortedHighest.slice(
        ...
        colorScale.domain([
            0,
            d3.median(worldData.features, (d) => d.properties.dataPoint),
            d3.max(worldData.features, (d) => d.properties.dataPoint),
        ]);
        create();
        showLegend();
    ...
    function grabInfo() {
        worldData.features.forEach((d) => {
            ...
            var firstSelection = worldData.data.filter((t) => {
